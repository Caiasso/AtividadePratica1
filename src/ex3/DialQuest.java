...
public class DialQuest extends JDialog implements ActionListener{
	...
	public static void main(String[] args) {
		try {
			DialQuest dialog = new DialQuest();
	...
	public DialQuest() {
		...
		setBounds(100, 100, 790, 310);
		{
			...
			mudaQuestao("1 - O Luciano é firmeza?", "Definitivamente sim", "Não", "Talvez", "Estou indeciso", "Não sei", numQuest);
	...
	public void mudaQuestao(String quest, String alt1, String alt2, String alt3, String alt4, String alt5, int numQuest)
	{
		...
		else if(e.getActionCommand().equals("Próxima Questão"))
		{
			if(! btnAlt1.isEnabled())
			{
				...
				case 1:
					numQuest ++;
					mudaBotoes();
					mudaQuestao("2 - Quais os quatro principais conceitos da Programação Orientada a Objetos?", "Ordenhar vacas; Andar em corda bamba; Dançar em circos; Pular corda com orangotangos", "Escrever lindos poemas; Conversar durante a aula; Ressitar ensinamentos de Mahatma Gandhi; Fazer bolos", "Abstração; Encapsulação; Herança; Polimorfismo", "Conversar com parentes; Comer bolinho de chuva; Encontrar pessoas queridas; Lembranças da infância", "Ir pra cima; Ir pra baixo; Ir pra um lado; Ir pro outro", numQuest);
					break;
					
				case 2:
					numQuest ++;
					mudaBotoes();
					mudaQuestao("3 - Qual dessas é uma linguaquem de programação?", "Ben & Jerry's Cookies n' Cream", "Python", "Carlo Bauducco, criador da Bauducco", "Ranking das melhores pantufas da Escócia", "Arapaima Gigas - nome ciêntífico do peixe Pirarucu", numQuest);
					break;
					
				case 3:
					numQuest ++;
					mudaBotoes();
					mudaQuestao("4 - Quais são as chances do filme Motoqueiro Fantasma ser real?", "Altas, altíssimas", "Altas, mas não são maiores que as chances do Nemo do filme Procurando Nemo ser real", "Considerando a estação atual (outono), são altas", "Praticamente impossível", "É REAL, EU JÁ VI O MOTOQUEIRO FANTASMA VOANDO POR AÍ PEGANDO FOGO E TUDO!!!!!!", numQuest);
					break;
					
				case 4:
					int resposta = JOptionPane.showConfirmDialog(panel, "Você acertou "+Integer.toString(numAcertos)+" questões", "Seus Resultados", JOptionPane.DEFAULT_OPTION, 1);
